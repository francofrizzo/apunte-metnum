...
\neq 0$. En tal caso, el costo de hallarla será lineal en la cantidad de
ecuaciones, es decir, $\ord{n}$.
...
$\lambda^{(k)}_i$ elegido convenientemente para cada fila $i$. Como, para todo
$i = k+1,\dots,n$, los coeficientes de la fila $i$-ésima quedarán
\[ a^{(k)}_{i,j} = a^{(k-1)}_{i,j} - \lambda^{(k)}_i \cdot a^{(k-1)}_{k,j} \]
y se quiere dejar un $0$ en la columna $k$-ésima, es decir, $a^{(k)}_{i,k} =
0$, debe tomarse, para cada fila $i$, el multiplicador
\[ \lambda^{(k)}_i = \frac{a^{(k-1)}_{i,k}}{a^{(k-1)}_{k,k}} \]
